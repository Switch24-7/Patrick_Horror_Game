...
public class CameraFollowPlayerLevel2 : MonoBehaviour {
	Transform player;
	[HideInInspector]
	public string Location = "storage";
	...
	void Update () {
		if (Location == "exit") {
			...
			this.transform.position = new Vector3 (Mathf.Clamp (player.position.x, 31.15f, 36.48f), Mathf.Clamp (player.position.y , -1.99f, 1.98f), this.transform.position.z);
		}
	}
}
